...
def extract_payload(result):
    """Retourne un dict qui contient au moins content (texte) + raw."""
    content_texts = []
    for c in getattr(result, "content", []) or []:
        txt = getattr(c, "text", None)
        content_texts.append(txt if txt is not None else str(c))
    return {"content": content_texts}
...
with st.form("gen"):
    ...
    with colA:
        max_tech = st.slider("Tech max", 10, 80, 30)
    with colB:
        sort = st.selectbox("Sort technos", ["intensity", "date"], index=0)
    with colC:
        categories = st.text_input("Categories (comma)", placeholder="Cloud, Data, Security")
...
if submitted:
    ...
    if not domain:
        ...
        try:
            with st.status("Generating first-touch pack...", expanded=True) as status:
                status.write("1) MCP: firmographics")
                firmo_res = run_async(mcp_call("company_firmographic", {"companyDomain": domain}))
                firmo = extract_payload(firmo_res)
                st.progress(30)

                status.write("2) MCP: technographics")
                ...
                status.write("3) LLM: generating pack")
            ...
            with tab1:
                ...
                c1.metric("Employees", firmo.get("employee_size", "N/A"))
                c2.metric("Revenue", firmo.get("revenue", "N/A"))
                c3.metric("HQ", firmo.get("hq", "N/A"))
                c4.metric("Industry", firmo.get("industry", "N/A"))
